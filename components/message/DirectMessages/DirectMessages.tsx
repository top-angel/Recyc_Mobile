...
import { accountTruncate } from "../utils/accountTruncate";
import { didToAddress, sleep } from "../utils/_utils";
...
import { providerToBrowserProvider } from "../../../components/MainDiscussion/genericBrowserWallet";
...
const CONVERSATION_CONTEXT =
  process.env.NEXT_PUBLIC_ORBIS_CONTEXT || "recyclium_market";
...
function OrbisProvider({ children }: { children: ReactNode }): ReactElement {
  ...
  const connectLit = async () => {
    ...
    const privyProvider = await wallet.getEthereumProvider();
    const orbisProvider = providerToBrowserProvider(privyProvider);
    const res = await orbis.connectLit(orbisProvider);
  ...
  const createConversation = async (recipients: string[]) => {
    ...
    if (!(await orbis.isConnected())) return null;
  ...
  const getConversationTitle = async (conversationId: string) => {
    if (conversationId && conversations.length) {
      ...
      const did = conversation.recipients.find((recipient) =>
        recipient.startsWith(didPrefix),
      );

      const address = didToAddress(did);

      // // Get ens name if exists
      const ensName = accountTruncate(address);
